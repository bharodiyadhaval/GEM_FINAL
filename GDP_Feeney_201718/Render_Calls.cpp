...
extern cFBO pass1_cFBO;
extern cFBO pass2_cFBO;
...
{
	if ( ( ::pass1_cFBO.width != width ) || ( ::pass1_cFBO.height != height ) )
	{
		// Window size has changed, so resize the offscreen frame buffer object
		std::string error;
		if ( ! ::pass1_cFBO.reset( width, height, error ) )
		{
			std::cout << "In window_size_callback(), the pass1_cFBO.reset() call returned an error:" << std::endl;
			std::cout << "\t" << error << std::endl;
		}
		else
		{
			std::cout << "Offscreen pass1_cFBO now: " << width << " x " << height << std::endl;
		}
	}//if ( ( ::pass1_cFBO.width....

	if ( ( ::pass2_cFBO.width != width ) || ( ::pass2_cFBO.height != height ) )
	{
		// Window size has changed, so resize the offscreen frame buffer object
		std::string error;
		if ( ! ::pass2_cFBO.reset( width, height, error ) )
		{
			std::cout << "In window_size_callback(), the pass2_cFBO.reset() call returned an error:" << std::endl;
			std::cout << "\t" << error << std::endl;
		}
		else
		{
			std::cout << "Offscreen pass2_cFBO now: " << width << " x " << height << std::endl;
		}
	}//if ( ( ::pass1_cFBO.width....
...
//	}
	...
	// uniform sampler2D texSamp2D00;		// Represents a 2D image
		...
		GLint Camera_Shader_Position = glGetUniformLocation(::g_pShaderManager->getIDFromFriendlyName("mySexyShader"), "CurCameraPosition");
		glm::vec3 Cureye = ::g_pTheCamera->getEyePosition();
		glUniform3f(Camera_Shader_Position, Cureye.x, Cureye.y, Cureye.z);



		GLint blendlava1 = glGetUniformLocation(::g_pShaderManager->getIDFromFriendlyName("mySexyShader"), "blendlava1");

		GLint HeightMapBlend = glGetUniformLocation(::g_pShaderManager->getIDFromFriendlyName("mySexyShader"), "blendHeightMap");
		//blendHeightMap
		// HACK: Check for the mesh name

		if (::blendvar1 > 1)
		{
			//::blendvar1Sign = -abs(::blendvar1Sign);
			::blendvar1Sign = -abs(::blendvar1Sign);
		}
		if (::blendvar1 < 0)
		{
		...
		::blendvar1 += (float)blendvar1Sign*(float)::deltaTime/80.0f;
		glUniform1f(blendlava1, ::blendvar1);
		glUniform1f(HeightMapBlend, ::posBlend);


	GLint changehue = glGetUniformLocation(::g_pShaderManager->getIDFromFriendlyName("mySexyShader"), "currenttimeflag");
	// HACK: Check for the mesh name
	if (::colortime + 3.0f <= glfwGetTime() && ::HueShiftOn)
	{
		::colortime = (float)glfwGetTime();
		::hue = !::hue;
		if (::hue)
		{
			glUniform1i(changehue, 1);
		}
		else
		{
			glUniform1i(changehue, 2);
		}

	}
	if (!::HueShiftOn)
	{
		glUniform1i(changehue, 4);
	...
	GLint texHeightMap_UniLoc2 = glGetUniformLocation(curShaderProgID, "texHeightMap2");
	// HACK: Check for the mesh name
	if ( theMesh.name == "plane" )
...
//		glUniform1f(texHeightMap_UniLoc, texureUnit );
		...
		GLint textureNameID 
			= ::g_pTextureManager->getTextureIDFromTextureName("lava2_hm.bmp");

		GLint textureNameID2
			= ::g_pTextureManager->getTextureIDFromTextureName("lava1_hm.bmp");
...
//		glBindTexture(GL_TEXTURE_2D, textureNameID);
		glBindTexture(GL_TEXTURE_2D, textureNameID);
		glUniform1f(texHeightMap_UniLoc, 31);

		glActiveTexture(GL_TEXTURE24);
		// Connect the TEXTURE to the TEXTURE UNIT
		//		glBindTexture(GL_TEXTURE_2D, textureNameID);
		glBindTexture(GL_TEXTURE_2D, textureNameID2);
		// Now assign the sampler to the texture Unit
		glUniform1f(texHeightMap_UniLoc2, 24 );		// Because GL_TEXTURE31

	//	GLint highmapvariable = glGetUniformLocation(curShaderProgID, "texHeightMap");
		//glUniform1i(highmapvariable, pass2unit);
	...
	GLint texNormalMap = glGetUniformLocation(curShaderProgID, "texNormalMap12");
	GLint texNormalMapImage = glGetUniformLocation(curShaderProgID, "texNormalMapImage12");
	GLint bIsNormalMap = glGetUniformLocation(curShaderProgID, "IsNormalMap");
	if (theMesh.name == "Mountain")
	{	// YES! It's the height map!
		glUniform1f(bIsNormalMap, GL_TRUE);
		// Also set the texture in the VERTEX shader...
		// What texture Unit was the height map assigned to?
		//		std::map<std::string, CTexUnitInfoBrief>::iterator itTexHM 
		//					= mapTexNameToTexUnit.find("height_map_norway-height-map-aster-30m.bmp");
		...
		// This is the TEXTURE ID, NOT the "texture UNIT" ID
		GLint ntextureNameID1= ::g_pTextureManager->getTextureIDFromTextureName("Nimage.bmp");
		//texNormalMapImage
		// Pick a texture unit we are not likely using elswhere
		glActiveTexture(GL_TEXTURE29);
		// Now assign the sampler to the texture Unit
		glUniform1f(texNormalMapImage, 29);
		glBindTexture(GL_TEXTURE_2D, ntextureNameID1);

		GLint ntextureNameID
			= ::g_pTextureManager->getTextureIDFromTextureName("Nmap.bmp");
		glActiveTexture(GL_TEXTURE28);
		// Connect the TEXTURE to the TEXTURE UNIT
		//		glBindTexture(GL_TEXTURE_2D, textureNameID);
		glUniform1f(texNormalMap, 28);		// Because GL_TEXTURE31

		glBindTexture(GL_TEXTURE_2D, ntextureNameID);
		// Now assign the sampler to the texture Unit
	...
	{
		glUniform1f(bIsNormalMap, GL_FALSE);
	}
	// ***************************************
	...
	GLint drawred_uniloc = glGetUniformLocation(curShaderProgID, "drawred");
	if (pTheGO->drawred==1)
	{
		glUniform1f(drawred_uniloc, GL_TRUE);
	}
	else
	{
		glUniform1f(drawred_uniloc, GL_FALSE);
	}

	GLint uniLoc_bIsPortal = glGetUniformLocation(curShaderProgID, "IsAPortal");
	if (pTheGO->bIsPortal)
	{
		glUniform1f(uniLoc_bIsPortal, GL_TRUE);
	}
	else
	{
		glUniform1f(uniLoc_bIsPortal, GL_FALSE);
	}

	GLint uniLoc_bIsBall = glGetUniformLocation(curShaderProgID, "IsABall");
	if (pTheGO->bIsBall)
	{
		glUniform1f(uniLoc_bIsBall, GL_TRUE);
	}
	else
	{
		glUniform1f(uniLoc_bIsBall, GL_FALSE);
	}

	//sobelEnable
	GLint uniLoc_bIsSobel = glGetUniformLocation(curShaderProgID, "sobelEnable");
	if (::sobelEffect)
	{
		glUniform1f(uniLoc_bIsSobel, GL_TRUE);
	}
	else
	{
		glUniform1f(uniLoc_bIsBall, GL_FALSE);

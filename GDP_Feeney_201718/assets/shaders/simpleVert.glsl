...
//	}
	...
	if ( bIsHeightMap )
	{
		...
		vec4 heightChange2XYZ = texture( texHeightMap2, uvX2.xy );	
		if( blendHeightMap*heightChangeXYZ.r + (1-blendHeightMap)*heightChange2XYZ.r <=0.7)
		{
				vertPosition.y += blendHeightMap* blendlava1 *1.5f* heightChangeXYZ.r  +  (1-blendHeightMap)*blendlava1 *1.5f* heightChange2XYZ.r ;
		}
		else
		{
		vertPosition.y += blendHeightMap*heightChangeXYZ.r + (1-blendHeightMap)*heightChange2XYZ.r ;
		}
	...
	if ( ! bIsASkinnedMesh )
	{
		...
		fVertNormal = vec3( mWorldInvTranspose * vec4(vNorm, 1.0f) ).xyz;	
			...
			vec3 vertexNormal_cameraspace = mv33 * normalize(vNorm);
			vec3 vertexTangent_cameraspace = mv33 * normalize(vTangent);
			vec3 vertexBitangent_cameraspace = mv33 * normalize(vBitangent);
			...
			vec3 lightposition;
			lightposition = vec3(0,-200,0);

			vec3 LightDirection_tangentspace = TBN * lightposition;
			vec3  EyeDirection_tangentspace =  TBN * CurCameraPosition;

...
uniform int currenttimeflag;
uniform bool IsNormalMap;
uniform sampler2D texNormalMap12;
uniform sampler2D texNormalMapImage12;
uniform bool IsAPortal;
uniform bool IsABall;
uniform bool drawred;
//uniform vec4 diffuseColour;		// New variable
uniform bool sobelEnable;
...
uniform int prakash;			// Now there are 3 passes, 0 to 2
...
{	
	...
	//		fragOut.colour.rgb *= 1.5f;	// Room too bright
		...
		if(IsNormalMap)
		{
			...
			FBOout_colour.rgb = normalize(texture( texNormalMap12, fUV_X2.xy ).rgb*2.0 - 1.0);
			//FBOout_colour.rgb = texture( texNormalMap12, fUV_X2.xy ).rgb;

			//FBOout_colour.rgb = vec3(1,0,0);
			FBOout_colour.rgb += texture( texNormalMapImage12, fUV_X2.xy ).rgb;
			return;
		}
			...
			if( drawred)
		{
			FBOout_colour = vec4(  1.0f,0.0f,0.0f, 1.0f);
		 return;
		}
		if( IsAPortal)
		{
					...
					//vec4 top         = texture(fullRenderedImage2D, vec2(fUV_X2.x, fUV_X2.y + 1.0 / 200.0));
					//vec4 bottom      = texture(fullRenderedImage2D, vec2(fUV_X2.x, fUV_X2.y - 1.0 / 200.0));
					//vec4 left        = texture(fullRenderedImage2D, vec2(fUV_X2.x - 1.0 / 300.0, fUV_X2.y));
					//vec4 right       = texture(fullRenderedImage2D, vec2(fUV_X2.x + 1.0 / 300.0, fUV_X2.y));
					//vec4 topLeft     = texture(fullRenderedImage2D, vec2(fUV_X2.x - 1.0 / 300.0, fUV_X2.y + 1.0 / 200.0));
					//vec4 topRight    = texture(fullRenderedImage2D, vec2(fUV_X2.x + 1.0 / 300.0, fUV_X2.y + 1.0 / 200.0));
					//vec4 bottomLeft  = texture(fullRenderedImage2D, vec2(fUV_X2.x - 1.0 / 300.0, fUV_X2.y - 1.0 / 200.0));
					//vec4 bottomRight = texture(fullRenderedImage2D, vec2(fUV_X2.x + 1.0 / 300.0, fUV_X2.y - 1.0 / 200.0));
					//vec4 sx = -topLeft - 2 * left - bottomLeft + topRight   + 2 * right  + bottomRight;
					//vec4 sy = -topLeft - 2 * top  - topRight   + bottomLeft + 2 * bottom + bottomRight;
					//vec4 sobel = sqrt(sx * sx + sy * sy);
					//SOBELCOLOR.rgb = sobel.rgb;
	...
	//		fragColourOut[0] = vec4(skyRGBA.rgb, 1.0f);		//gl_FragColor = skyRGBA;
			//FBOout_colour = vec4( vec3( (skyRGBA.r + skyRGBA.g + skyRGBA.b)/3, (skyRGBA.r + skyRGBA.g + skyRGBA.b)/3, (skyRGBA.r + skyRGBA.g + skyRGBA.b)/3  ), 1.0f);
			FBOout_colour  = vec4( vec3( skyRGBA.r, skyRGBA.g, skyRGBA.b), 1.0);
			FBOout_vertexWorldPos.xyz = fVecWorldPosition.xyz;
			FBOout_normal.a = DONT_CALCULATE_LIGHTING;
...
//
	...
	if(FBOout_colour.r <= 0.3)
	{
	//vec3 temprgb;
	//temprgb.rgb = vec3( (FBOout_colour.r + FBOout_colour.g + FBOout_colour.b)/3, (FBOout_colour.r + FBOout_colour.g + FBOout_colour.b)/3, (FBOout_colour.r + FBOout_colour.g + FBOout_colour.b)/3);
	//		FBOout_colour.rgb = temprgb.rgb;
	}
	//fboR = FBOout_colour.r;



	if(currenttimeflag==1)
	{
		FBOout_colour.rgb = vec3(FBOout_colour.b, FBOout_colour.g, FBOout_colour.r);
	}
	else if(currenttimeflag==2)
	{
			FBOout_colour.rgb = vec3(FBOout_colour.r, FBOout_colour.b, FBOout_colour.g);
	}
	else
	{
	FBOout_colour.rgb = vec3(FBOout_colour.r, FBOout_colour.g, FBOout_colour.b);
	}
...
// **************************************************************************
	...
	vec3 old_FBO_color;
		...
		if( sobelEnable)
		{
			vec4 top         = texture(texFBOColour2D, vec2(textCoords.x, textCoords.y + 1.0 / 200.0));
			vec4 bottom      = texture(texFBOColour2D, vec2(textCoords.x, textCoords.y - 1.0 / 200.0));
			vec4 left        = texture(texFBOColour2D, vec2(textCoords.x - 1.0 / 300.0, textCoords.y));
			vec4 right       = texture(texFBOColour2D, vec2(textCoords.x + 1.0 / 300.0, textCoords.y));
			vec4 topLeft     = texture(texFBOColour2D, vec2(textCoords.x - 1.0 / 300.0, textCoords.y + 1.0 / 200.0));
			vec4 topRight    = texture(texFBOColour2D, vec2(textCoords.x + 1.0 / 300.0, textCoords.y + 1.0 / 200.0));
			vec4 bottomLeft  = texture(texFBOColour2D, vec2(textCoords.x - 1.0 / 300.0, textCoords.y - 1.0 / 200.0));
			vec4 bottomRight = texture(texFBOColour2D, vec2(textCoords.x + 1.0 / 300.0, textCoords.y - 1.0 / 200.0));
			vec4 sx = -topLeft - 2 * left - bottomLeft + topRight   + 2 * right  + bottomRight;
			vec4 sy = -topLeft - 2 * top  - topRight   + bottomLeft + 2 * bottom + bottomRight;
			vec4 sobel = sqrt(sx * sx + sy * sy);
			FBOout_colour.rgb = sobel.rgb;
				
		}
		else
		{
			old_FBO_color = FBOout_colour.rgb;
		}
